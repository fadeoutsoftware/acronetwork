...
import it.fadeout.acronetwork.viewmodels.StationTypeViewModel;
...
public class StationsService {
		...
		if (oConfObj != null)  {
			...
			for (SensorLinkConfig oLinkConfig : oConfig.getSensorLinks()) {
				...
				if (oLinkConfig.getCode().equals(sCode)) {
					...
					String sPath = Acronetwork.getSubPath(oLinkConfig.getFilePath(), oDate);
					...
					if (sPath != null) {
						...
						File oLastFile = Acronetwork.lastFileModified(sPath);
						...
						if (oLastFile != null) {
							...
							try {
								// Ok read sensors 
								aoSensors = (List<SensorViewModel>) Acronetwork.deserializeXMLToObject(oLastFile.getAbsolutePath());
							} catch (Exception e) {
								e.printStackTrace();
							}							
						}
	...
	public SensorListTableViewModel GetStationsListTable(@PathParam("sCode") String sCode, @HeaderParam("x-session-token") String sSessionId) {
		...
		if (oConfObj != null)  {
			...
			try {
				// Ok read sensors 
				oTable = (SensorListTableViewModel) Acronetwork.deserializeXMLToObject(sPath);
			} catch (Exception e) {
				e.printStackTrace();
			}							
		}
